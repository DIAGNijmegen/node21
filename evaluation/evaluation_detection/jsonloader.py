...
class NODE21Loader(FileLoader):
    def load(self, *, fname: Path):
        ...
        if str(fname).endswith(".json"):
            if (('predictions') in str(fname) or ('reference') in str(fname)):
                print('fname ', fname)
                ...
                for e in entries:
                    ...
                    outputs = e['outputs']
                    for outp in outputs:
                        if outp["interface"]["slug"] == "nodules":
                            print('nodules output found')
                            for i in outp["value"]["boxes"]:
                                slice_nb = None
                                probability = None
                                x_min = None
                                y_min = None
                ...
                print('file is not predictions or reference')
